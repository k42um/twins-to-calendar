...
上記の例では「4月30日（木）は水曜日の時間割で授業を行う」を意味します。これにより：

- 木曜日の授業は 4/30 に開催されない（`EXDATE` で除外）
- 水曜日の授業が 4/30 に追加される（振替 `VEVENT` を生成）
...
- 繰り返し: `RRULE:FREQ=WEEKLY` でターム期間内を週次繰り返し
- 祝日・振替元曜日: `EXDATE` で除外
- 振替授業日: 個別の `VEVENT` として追加
- 予定タイトル: 科目名
- 説明欄: 担当教員名
- 場所: 教室名（KdB データがある場合）、なければ授業形態
...
├── popup.js            # ポップアップのロジック（ICS 生成含む）
├── options.html        # 設定ページ（KdB 取り込み）
├── options.js          # 設定ページのロジック（xlsx 解析）
├── calendars/
│   ├── 2026.json       # 2026年度のターム・祝日・振替設定
│   └── 2027.json       # （必要に応じて追加）
└── icons/
    ├── icon16.png
    ├── icon48.png
